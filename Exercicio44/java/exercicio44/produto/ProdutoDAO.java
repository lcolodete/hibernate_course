...
{	
	...
	{	try
		{	Session sessao = HibernateUtil.getSession();
			...
			Produto umProduto = (Produto)sessao
				.load(Produto.class, new Long(numero), LockMode.UPGRADE);
			return umProduto;
		} 
	...
	{	try
		{	Session sessao = HibernateUtil.getSession();
			...
			String busca = "from Produto p " +
			               "left outer join fetch p.lanceVencedor " +
			               "where p.id = :id";
	...
	{	
		...
		{	Session sessao = HibernateUtil.getSession();
			...
			String busca = "select max(l.valor) " +
			               "from Lance l " +
			               "where l.produto = :produto"; 

			Double valorMaiorLance = (Double) sessao
					.createQuery(busca)
