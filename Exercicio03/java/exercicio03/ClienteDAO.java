...
{	
	...
	{	Session sessao = null;
		...
		{	sessao = FabricaDeSessoes.criarSessao();
			...
			// Neste exemplo, o objeto persistente cliente está sendo removido
			// do banco de dados. Após a remoção ele se torna um objeto 
			// transiente e, em seguida, é destruído pelo coletor de lixo.
			
			Cliente umCliente = (Cliente)sessao
						.load(Cliente.class, new Long(numero));

			// Neste caso o acesso a banco de dados acontece e a exceção 
			// ObjectNotFoundException é propagada quando o método delete é 
			// executado. Um select é emitido no momento que o delete() é
