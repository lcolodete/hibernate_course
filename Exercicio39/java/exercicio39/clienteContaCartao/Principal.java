...
import exercicio39.cliente.*;
import exercicio39.conta.*;
import exercicio39.cartao.*;
...
public class Principal
{
	public static void main(String[] args)
	{
		...
		while (continua)
		{
			...
			System.out
					.println("2. Cadastrar um novo cartao para um cliente e uma conta");
			System.out
					.println("3. Cadastrar uma nova conta e cartao para um determinado cliente");
			System.out
					.println("4. Cadastrar um novo cliente com um novo cartao em determinada conta");
			...
			case 1:
			{
				...
				ClienteContaCartao ccc = new ClienteContaCartao(umCliente, umaConta,
						umCartao);

				/* ==> */clienteContaCartaoAppService.inclui(ccc);

				System.out.println('\n' + "Cliente " + umCliente.getId() + " Conta "
						+ umaConta.getId() + " Cartao " + umCartao.getId()
			...
			{ // Cadastrar um novo cartao para um cliente e uma conta
				...
				/* ==> */Cliente umCliente = ((ClienteContaCartao) cccs.get(0))
						.getCliente();
				/* ==> */Conta umaConta = ((ClienteContaCartao) cccs.get(0))
						.getConta();
			...
			case 4: // Cadastrar um novo cliente com um novo cartao em determinada
							// conta
			{
				int idConta = Console.readInt('\n' + "Digite o numero da conta: ");
				...
				try
				/* ==> */{
					umaConta = contaAppService.recuperaUmaConta(idConta);
				}
				catch (AplicacaoException e)
				{
					System.out.println('\n' + e.getMessage());
			...
			case 9:
			{
				...
				if (cccs.size() != 0)
				{
					...
					for (Iterator it = cccs.iterator(); it.hasNext();)
					{
						...
						System.out.println('\n' + "Cliente numero = "
								+ ccc.getCliente().getId() + "  Conta numero = "
								+ ccc.getConta().getId() + "  Cartao numero = "
								+ ccc.getCartao().getId());
					}
				}
				else
